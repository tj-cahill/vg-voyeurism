...
table(qualtrics$gender) %>% 
  rbind(prop.table(table(qualtrics$gender)) * 100) %>% 
  t() %>% kable(col.names = c("Frequency", "%"), digits = 3)
```
...
table(qualtrics$firstLang) %>% 
  rbind(prop.table(table(qualtrics$firstLang)) * 100) %>% 
  t() %>% kable(col.names = c("Frequency", "%"), digits = 3)
```
## Measures
