...
H3 is that *the gender of the streamer will affect the perceived efficacy of the streamer*, given identical gameplay performance. This can also be assessed with an independent-sample T test.
...
# First test for homogeneity of variance
bartlett.test(playerEfficacy ~ stimGender, h3.data) -> h3.bartlett_test
h3.var_equal <- if(h3.bartlett_test$p.value > 0.05) TRUE else FALSE
print(h3.bartlett_test)

t.test(playerEfficacy ~ stimGender, h3.data, var.equal = h3.var_equal, 
       alternative = "two.sided") -> h3.test # Two-sided test
print(h3.test)
```

The hypothesis is **not supported**: *t*(`r h3.test$parameter`) = `r h3.test$statistic %>% round(3)`, *p* = `r h3.test$p.value %>% round(3)`